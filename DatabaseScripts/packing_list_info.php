<?php
	...
	if(isset($_POST['add']))
	{
		...
		$sql = 'INSERT INTO project_equipment (project_id, equipment_id, revision_number) VALUES';
		$changed = false;
	...
	{
		$project_id = $_POST['project_id'];
		$sql = 'DELETE FROM project_equipment WHERE';
		foreach($_POST['equipment_id'] as $equipment_id)
		{

			if($_POST[''.$equipment_id] == 'on')
			{
				$sql .= ' project_id='.$project_id.' AND equipment_id='.$equipment_id.' OR';
			}
		}
		$sql .= ' FALSE';
		$query = $GLOBALS['pdo']->prepare($sql);
